...
declare const Editor: {
  ...
  Project: { path: string };
  Message: {
    request: (scope: string, method: string, ...args: unknown[]) => Promise<unknown>;
  };
};
...
let resourceIndex: ResourceIndex | null = null;
...
export async function load(): Promise<void> {
  ...
  try {
    ...
    // 2. 初始化资源索引
    resourceIndex = new ResourceIndex(Editor.Project.path);
    // 异步全量扫描
 ...
 */
function ensureComponentCache(): void {
  ...
  if (fs.existsSync(bundledPath)) {
    try {
      ...
      console.warn('[Comdr Bridge] Failed to copy component-cache.json:', (e as Error).message);
    }
  } else {
 ...
 */
async function ensureEngineSchema(): Promise<void> {
  const projectPath = Editor.Project.path;
  ...
  try {
    ...
    process.stderr.write(`[bridge] engine-schema dump failed: ${(e as Error).message}\n`);
  }
}
...
export function open(assetPath: string): Promise<unknown> {
...
const _openDocs = new Map<string, Document>();
let _activeDocPath: string | null = null;

function getDoc(): Document | null {
  if (!_activeDocPath) return null;
  return _openDocs.get(_activeDocPath) || null;
}

function setDoc(path: string, doc: Document): void {
...
function clearDoc(path?: string): void {
  if (path) {
    ...
    if (_activeDocPath === path) _activeDocPath = null;
  } else {
    _openDocs.clear();
    _activeDocPath = null;
  }
...
async function runTaskCardFromEditor(taskCard: {
  ...
  if (_busy) return { ok: false, error: 'Bridge busy — concurrent request rejected', code: 'BR_BUSY' };
  _busy = true;
  ...
  if (!_schemaDumped) {
    ...
    ensureEngineSchema().then(() => {
      ...
      try { fs.writeFileSync(markerPath, 'ok at ' + new Date().toISOString(), 'utf8'); } catch (_) { /* */ }
    }).catch((e: Error) => {
      const ep = path.join(Editor.Project.path, 'temp', 'comdr', 'schema-error.txt');
      try { fs.writeFileSync(ep, e.message + '\n' + (e.stack || ''), 'utf8'); } catch (_) { /* */ }
    });
  }
  ...
  switch (type) {
    case 'probe': {
      const doc = getDoc();
      const probe = new ProbeV2(projectPath, doc);
      if (resourceIndex) probe.setResourceIndex(resourceIndex);
      const kind = (payload.probeType || payload.kind || 'assets') as string;
      return probe.handle({
        kind: kind as 'project-summary' | 'assets' | 'asset' | 'asset-search' | 'find-in-doc' | 'node-detail' | 'document-serialize' | 'schema' | 'scripts' | 'console' | 'property',
        path: payload.path as string | undefined,
        name: payload.name as string | undefined,
        pattern: payload.pattern as string | undefined,
        fileId: payload.nodeUuid as string | undefined,
        componentType: payload.component as string | undefined,
        property: payload.property as string | undefined,
    ...
    case 'write': {
      ...
      const writeResult = await writer.writeAsset(payload) as Record<string, unknown>;
      // N14: 如果 write 路径匹配当前打开文档，自动重新加载文档
      const wrotePath = writeResult.path as string | undefined;
      if (wrotePath && _activeDocPath && wrotePath === path.basename(_activeDocPath)) {
        const reopen = Document.open(projectPath, wrotePath);
        if (reopen.ok) setDoc(wrotePath, reopen.doc);
      }
      return writeResult;
    }
    case 'open': {
      const openPath = payload.path as string;
      const result = Document.open(projectPath, openPath);
      if (result.ok) {
    ...
    case 'save': {
      ...
      if (!fs.existsSync(path.dirname(savePath))) {
        return { ok: false, error: `Save directory does not exist: ${path.dirname(savePath)}`, code: ERR_BR_NO_DOC };
      }
      const content = doc.serialize();
      // 诊断：写盘前检查所有脚本组件的属性（读 _key 下划线前缀，Cocos 标准存储）
      try {
        const parsed = JSON.parse(content);
        const scriptComps: Array<Record<string, unknown>> = [];
        for (const o of parsed) {
          if (o && typeof o === 'object' && o.__type__ && typeof o.__type__ === 'string'
            ...
            for (const k of Object.keys(o)) {
              if (k === '__type__' || k === '__id__' || k === 'node' || k === '__prefab' || k === '_name' || k === '_objFlags' || k === '_id' || k === '_enabled' || k === '__editorExtras__' || k === '_rawProps') continue;
              props[k] = (o as Record<string, unknown>)[k];
            }
