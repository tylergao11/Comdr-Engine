...
exports.diffPrefab = diffPrefab;
exports.diffAllSnapshots = diffAllSnapshots;
...
function diffPrefab(path, before, after) {
    ...
    for (const fileId of beforeIds) {
        ...
        const beforeComps = new Map();
        for (const c of before.components)
            beforeComps.set(c.type, c);
        const afterComps = new Map();
        for (const c of after.components)
            afterComps.set(c.type, c);
        // Components removed
...
/** 批量 diff，返回所有有差异的结果 */
function diffAllSnapshots(snapshots) {
    return snapshots.map((s) => diffPrefab(s.path, s.before, s.after));
}
/** 汇总差异为多行文本 */
function formatDiffResults(diffs) {
    ...
    for (const diff of diffs) {
        ...
        for (const type of ['added', 'removed', 'modified']) {
...
function extractNodes(json) {
    ...
    for (let i = 0; i < json.length; i++) {
        ...
        for (const ref of compRefs) {
            ...
            if (compIdx == null || compIdx < 0 || compIdx >= json.length)
                continue;
            const compObj = json[compIdx];
            if (!compObj || compObj.__deleted__ || !compObj.__type__)
                continue;
            if (compObj.__type__ === 'cc.PrefabInfo' || compObj.__type__ === 'cc.CompPrefabInfo')
                continue;
            // Component fileId
            ...
            for (const key of Object.keys(compObj)) {
                if (SKIP_KEYS.has(key))
                    continue;
                props[key] = safeValue(compObj[key]);
            }
            components.push({
                fileId: compFileId,
...
function shortType(fullType) {
    if (fullType.startsWith('cc.'))
        return fullType.slice(3);
    return fullType;
}
/** 格式化值为可读字符串（控制长度） */
function formatVal(val) {
    ...
    if (typeof val === 'string') {
        if (val.length > constants_1.DIFF_VALUE_MAX)
            return `"${val.slice(0, constants_1.DIFF_VALUE_MAX - 2)}..."`;
        return `"${val}"`;
    }
    if (typeof val === 'object') {
        const s = JSON.stringify(val);
        if (s.length > constants_1.DIFF_OBJ_MAX)
            return s.slice(0, constants_1.DIFF_OBJ_MAX - 2) + '...';
        return s;
    }
    return String(val);
}
...
//# sourceMappingURL=prefab-diff.js.map